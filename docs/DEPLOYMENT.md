...
Use Node 22 LTS (`nvm use` if installed), `npm ci`, `npm run dev`. Validate with `npm run check` and `npm run test:e2e` after building (first install browser: `npx playwright install chromium`). Build output: `dist/`.
...
Created and verified during initial development: `fractions-fighter-verdant`. Configuration is committed in `.firebaserc`. No billing was enabled. Hosting release status is in `docs/memory/STATE.md`.
